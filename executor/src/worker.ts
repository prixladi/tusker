...
const getNextSchedule = (task: Task, queue: Queue) => {
  ...
  return now
    .add(
      Math.min(queue.maxBackoff, queue.minBackoff * task.retries + 1),
      'seconds'
    )
    .toDate();
};

const m = (message: string) => `[${config.queueName}] ${message}`;
...
if (!config.queueName) {
  log.error('Queue name was not provided exiting worker.');
  Deno.exit(1);
} else {
